...
function buildViewModelFromApi(raw: Record<string, unknown>): PlayerPageViewModel {
...
function buildViewModelFromMock(p: PlayerPageData): PlayerPageViewModel {
...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  const userId =
    ...
    (typeof user?.email === 'string' && user.email.trim() ? user.email.trim() : null);
  const isCoach = (user.groups ?? []).includes('coaches');
...
export default function PlayerStatsPage() {
  const { model, userId, userName, backendBaseUrl, isCoach, notesRecipientId } = useLoaderData<typeof loader>();
  ...
  useEffect(() => {
    if (!isCoach || !notesRecipientId) return;
    let cancelled = false;
    (async () => {
      setLoadingNotes(true);
      try {
        const r = await fetch(
          `${backendBaseUrl}/api/notes/by-recipient?recipientId=${encodeURIComponent(notesRecipientId)}`
        );
  ...
  }, [isCoach, notesRecipientId, backendBaseUrl]);

  const postNote = async () => {
    if (!noteDraft.trim() || !userId || !notesRecipientId) return;
    setSendingNote(true);
    try {
              ...
              <div className="managePlayersNotesList playerProfileNotesList">
                {loadingNotes ? (
                  ...
                  notes.map((n) => {
                    ...
                    const preview =
                      full.slice(0, NOTE_PREVIEW_CHARS).replace(/\s+\S*$/, '').trimEnd() || full.slice(0, NOTE_PREVIEW_CHARS);
                    return (
