...
type Player = { name: string; points: number; rebounds: number; assists: number; fgPercent: number; threePPercent: number; ftPercent: number; steals: number; blocks: number; turnovers: number; salary: number; };
type Team = { name: string; players: Player[]; };

const teams: Team[] = [
  // same teams & players as before
];
...
const CompareWithMoney: React.FC = () => {
  ...
  const sumStats = (team:Team) => team.players.reduce((acc,p)=>{ acc.points+=p.points; acc.rebounds+=p.rebounds; acc.assists+=p.assists; acc.fgPercent+=p.fgPercent; acc.threePPercent+=p.threePPercent; acc.ftPercent+=p.ftPercent; acc.steals+=p.steals; acc.blocks+=p.blocks; acc.turnovers+=p.turnovers; return acc;},{points:0,rebounds:0,assists:0,fgPercent:0,threePPercent:0,ftPercent:0,steals:0,blocks:0,turnovers:0});
    ...
    if(mode==="players"){
      ...
      return (
        <div className="roundedBox">
          <table>
            <thead><tr><th>Stat</th><th>{pA.name}</th><th>{pB.name}</th></tr></thead>
            <tbody>
      ...
      const tA = teams[teamAIndex], tB = teams[teamBIndex];
      const sA = sumStats(tA), sB = sumStats(tB);
      return (
        <div className="roundedBox">
          <table>
            <thead><tr><th>Stat</th><th>{tA.name}</th><th>{tB.name}</th></tr></thead>
            <tbody>
...
export default CompareWithMoney;